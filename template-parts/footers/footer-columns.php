 ...
 * Multi-column layout with logo, 2-3 menu columns, social links, and copyright bar
 *
...
<footer class="clesk-footer bg-[var(--color-surface-dark)] border-t border-[var(--color-border)]">
    <div class="max-w-[85rem] mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
            ...
            <!-- Column 4: Optional Menu 3 + Social Links -->
            <div>
                <?php if (has_nav_menu('footer_col_3')) : ?>
                    <?php
                    $menu_obj3 = isset($menu_locations) ? (isset($menu_locations['footer_col_3']) ? wp_get_nav_menu_object($menu_locations['footer_col_3']) : false) : false;
                    if (!$menu_obj3) {
                        $locs = get_nav_menu_locations();
                        $menu_obj3 = isset($locs['footer_col_3']) ? wp_get_nav_menu_object($locs['footer_col_3']) : false;
                    }
                    if ($menu_obj3) :
                    ?>
                        <h4 class="text-sm font-semibold text-[var(--color-heading)] uppercase tracking-wider mb-4">
                            <?php echo esc_html($menu_obj3->name); ?>
                        </h4>
                    <?php endif; ?>
                    <?php
                    wp_nav_menu(array(
                ...
                <?php if (!empty(array_filter($social_links))) : ?>
                    <div class="<?php echo has_nav_menu('footer_col_3') ? 'mt-6' : ''; ?>">
                        <h4 class="text-sm font-semibold text-[var(--color-heading)] uppercase tracking-wider mb-4">
